...
def castro(n: int, m: int, v: callable, original: Any) -> List[float]:
    ...
    for l in n_set:
        with mp.Pool() as pool:
            results = pool.starmap(
                estimate_s_2_il,
                [[l, i, n_set, v, original, m_exp_il] for i in n_set]
            )
            for result in results:
                l_proc, i_proc, sh_l_i_proc, s_2_il_proc = result
    ...
    for l in n_set:
        with mp.Pool() as pool:
            ...
            for result in results:
                l_proc, i_proc, sh_l_i_proc = result
                sh_l_i[l_proc][i_proc] = sh_l_i_proc
    ...
    for i in n_set:
        sh_st_opt_i[i] = float_round_to_zero(sh_st_opt_i[i] / Decimal(n))
...
def castro_interaction_index(n: int, m: int, v: callable, original: Any) -> List[List[float]]:
        ...
        with mp.Pool() as pool:
            results = pool.starmap(
                ...
                [
                    ...
                    for j in n_set for i in n_set
                    if j != i
                ]
            )
            for result in results:
                l_proc, i_proc, j_proc, sh_l_ij_proc, s_2_ijl_proc = result
    ...
    for l in n_set:
        with mp.Pool() as pool:
            ...
            for result in results:
                l_proc, i_proc, j_proc, sh_l_ij_proc = result
                sh_l_ij[l_proc][i_proc][j_proc] = sh_l_ij_proc
    ...
    for i in n_set:
        for j in n_set:
            if j != i:
                sh_st_opt_ij[i][j] = float_round_to_zero(sh_st_opt_ij[i][j] / Decimal(n))
